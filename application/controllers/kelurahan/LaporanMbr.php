...
class LaporanMbr extends CI_Controller{
    ...
    public function LaporanMbr($tanggal = NULL)
    {
        $data['title'] = "Laporan Data MBR";
        $kec=$this->session->userdata('kec');
        $kelurahan = $this->session->userdata('kel');
        $kel   = (strstr($kelurahan, "%20"))?str_replace("%20", " ", $kelurahan):$kelurahan;
        $now = date("m-Y");
        $bulan_tahun = ($tanggal=='-' OR empty($tanggal))?NULL:$tanggal;
        ...
        $this->load->view('kelurahan/cetakMbr',$data);
    }

    public function LaporanPdf($tanggal=NULL)
    {
        ...
        $this->pdf->load_view('kelurahan/laporanPdfMbr', $data);
